...
def card(p, x, y, idx):
    ...
    repo = repo.rstrip("/")
    href = p.get("url", "").strip() or (f"https://github.com/{repo}" if repo else "#")
    card_label = repo or href.replace("https://", "").replace("http://", "").rstrip("/")
    ...
    a(f'<text x="16" y="19" font-size="10" fill="{MUTED}"><tspan fill="{CYAN}">&#8226;</tspan> {esc(card_label)}</text>')
